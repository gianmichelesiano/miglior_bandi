...
reqeust = requests.get(base_url)
testoHTML = reqeust.text
soup = BeautifulSoup(testoHTML, "html.parser")
elemnet_ul = soup.find_all("ul", attrs={"class": "ultimelist"})[0]
tutti_li = soup.find_all("li")

for uno in tutti_li:
    if "Contratti pubblici" in uno.get_text():
        
            link =  uno.a['href']
            data = uno.a['href'].split('ataPubblicazioneGazzetta=')[1].split('&numeroGazzetta')[0]
            cursore = conn.cursor()
