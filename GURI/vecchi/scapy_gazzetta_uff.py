...
mio =   risultati[0].a.get_text().strip().replace(chr(9),"").replace(chr(10),"")
valori =  mio.split("(")
ente = valori[0].strip()
...
link = base_url + risultati[0].a.get('href')
link = 'http://www.gazzettaufficiale.it//atto/contratti/caricaDettaglioAtto/originario;jsessionid=onj1Ah01LxRIcACAdYPGPw__.ntc-as4-guri2a?atto.dataPubblicazioneGazzetta=2017-01-11&atto.codiceRedazionale=TX17BFK454'
...
# importo
regex_importi = '[0-9]+[.][0-9]+[,][0-9]+'
p = re.compile(regex_importi)
print p.findall(testo_per_parser)

# mail
regex_mail = '[0-9]+[.][0-9]+[,][0-9]+'
p = re.compile(regex_mail)
print p.findall(testo_per_parser)

# sito www
regex_sito_www = '[w]{3}[.][a-z]+[.][a-z]+'
...
# date
reg_CPV = '\d{8}[ -12334567890]{1,2}'
p = re.compile(reg_CPV)
print p.findall(testo_per_parser)

# date
regex_CIG = '[0-9]{2}[A-Z0-9]{8}'
