...
lista_gia_inclusi = []
sql_valutati = "SELECT * FROM gare.GARE_INFO;"
cursore = conn.cursor()
lista_valutati = []
...
for rec in records_provincie:
    try:
            ...
            regione = record_regione[1].upper()
            #print regione
            ...
            for riga in righe:
                ...
                if (len(celle)>7):
                    ...
                    tutta_italia = celle[5].get_text().strip()
                    #print tutta_italia
                    if 'italia' in tutta_italia:
                            provincia = "TT"
                    ...
                    if cig not in lista_gia_inclusi and len(cig_exist) == 0:
                            ...
                            insert_gara_info(identificativo_gara, provenienza, cig, oggetto, tipologia, procedura, ente, citta, provincia, regione.upper(), importo, data_inserimento, data_pubblicazione, data_scadenza, cpv, categoria_prevalente, categorie_scorporabili, info_aggiuntive, download)

    except Exception as e:
            s = str(removeNonAscii(str(e)))
            print s
            #invia_mail("errore script INFO"+testo, removeNonAscii(s))
            pass
